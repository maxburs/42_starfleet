...
int main(void)
{
	...
	n = artCount(arts);
	test(arts, n, "Guernica");
	test(arts, n, "Mona Lisa");
	test(arts, n, "I dont exist");

	return (0);
}

int artCount(struct s_art **arts)
{
	int count;
...
void test(struct s_art **arts, int n, char *name)
{
	printf("price for the art \'%s\' is %d\n", name, searchPrice(arts, n, name));
}
