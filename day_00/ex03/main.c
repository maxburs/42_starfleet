...
int main(void)
{
	...
	dict = dictInit(500);
	addnArts(dict, arts, 5);
	while (fgets(art, sizeof(art), stdin))
	{
		newline = strchr(art, '\n');
		*newline = '\0';
		printf("price: %d\n", searchPrice(dict, art));
	}

	return (0);
}

void	addnArts(struct s_dict *dict, struct s_art **arts, size_t n)
{
	while (n)
	{
		n--;
		dictInsert(dict, arts[n]->name, arts[n]);
	}
}
